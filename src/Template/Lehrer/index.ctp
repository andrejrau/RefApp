<?php
/**
  * @var \App\View\AppView $this
  * @var \App\Model\Entity\Lehrer[]|\Cake\Collection\CollectionInterface $lehrer
  */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('New Lehrer'), ['action' => 'add']) ?></li>
        <li><?= $this->Html->link(__('List Arbeitsmaterialien'), ['controller' => 'Arbeitsmaterialien', 'action' => 'index']) ?></li>
        <li><?= $this->Html->link(__('New Arbeitsmaterialien'), ['controller' => 'Arbeitsmaterialien', 'action' => 'add']) ?></li>
        <li><?= $this->Html->link(__('List Klasse Fach'), ['controller' => 'KlasseFach', 'action' => 'index']) ?></li>
        <li><?= $this->Html->link(__('New Klasse Fach'), ['controller' => 'KlasseFach', 'action' => 'add']) ?></li>
    </ul>
</nav>
<div class="lehrer index large-9 medium-8 columns content">
    <h3><?= __('Lehrer') ?></h3>
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th scope="col"><?= $this->Paginator->sort('id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('vorname') ?></th>
                <th scope="col"><?= $this->Paginator->sort('nachname') ?></th>
                <th scope="col"><?= $this->Paginator->sort('telefonnummer') ?></th>
                <th scope="col"><?= $this->Paginator->sort('email') ?></th>
                <th scope="col"><?= $this->Paginator->sort('mentor') ?></th>
                <th scope="col"><?= $this->Paginator->sort('created') ?></th>
                <th scope="col"><?= $this->Paginator->sort('modified') ?></th>
                <th scope="col" class="actions"><?= __('Actions') ?></th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($lehrer as $lehrer): ?>
            <tr>
                <td><?= $this->Number->format($lehrer->id) ?></td>
                <td><?= h($lehrer->vorname) ?></td>
                <td><?= h($lehrer->nachname) ?></td>
                <td><?= h($lehrer->telefonnummer) ?></td>
                <td><?= h($lehrer->email) ?></td>
                <td><?= h($lehrer->mentor) ?></td>
                <td><?= h($lehrer->created) ?></td>
                <td><?= h($lehrer->modified) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $lehrer->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $lehrer->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $lehrer->id], ['confirm' => __('Are you sure you want to delete # {0}?', $lehrer->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->first('<< ' . __('first')) ?>
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
            <?= $this->Paginator->last(__('last') . ' >>') ?>
        </ul>
        <p><?= $this->Paginator->counter(['format' => __('Page {{page}} of {{pages}}, showing {{current}} record(s) out of {{count}} total')]) ?></p>
    </div>
</div>
